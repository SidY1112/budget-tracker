...
// Fetches all budgets for the user with their category name so the dashboard can display them without a second query
export async function getBudgets(userId) {
  const supabase = createSupabaseServerClient()

  const { data, error } = await supabase
    .from('budgets')
    .select('id, monthly_limit, expiration_date, categories(name, icon)')
    .eq('user_id', userId)
